...
export const deleteBook = async (bookId: string) => {
    try {
        ...
        // Best-effort blob cleanup - a failed delete here shouldn't block removing
        // the DB records, since an orphaned blob is far less harmful than a book
        ...
        if (blobUrls.length > 0) {
            try {
                ...
                console.error(
                    "Error deleting blob files for book",
                    bookId,
                    blobError,
                );
            }
