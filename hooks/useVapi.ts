...
export function useVapi(book: IBook) {
    ...
    const [capReachedMessage, setCapReachedMessage] = useState<string | null>(
        null,
    );
    ...
    useEffect(() => {
            ...
            "call-start": () => {
                ...
                timerRef.current = setInterval(() => {
                    if (startTimeRef.current) {
                        ...
                        // Check duration limit - end the call, but surface this as a
                        // dismissible in-place upsell (capReachedMessage) rather than
                        // a toast + forced redirect, since this is expected plan
                        // behavior, not a failure.
                        if (newDuration >= maxDurationRef.current) {
                            getVapi().stop();
                            setCapReachedMessage(
                                `You've reached the ${Math.floor(
                                    maxDurationRef.current / SECONDS_PER_MINUTE,
                                )}-minute session limit for your plan.`,
                            );
                        }
                    }
                }, TIMER_INTERVAL_MS);
        ...
        return () => {
            // End active session on unmount. durationRef is a useLatestRef - it's
            // synced to the current duration on every render specifically so this
            // cleanup reads the freshest value, not a stale one from when the effect
            // was set up, which is what this rule normally guards against.
            if (sessionIdRef.current) {
                getVapi().stop();
                // eslint-disable-next-line react-hooks/exhaustive-deps
                const finalDuration = durationRef.current;
                endVoiceSession(sessionIdRef.current, finalDuration).catch(
                    (err) =>
                        console.error(
    ...
    const dismissCapReached = useCallback(() => {
        setCapReachedMessage(null);
    }, []);
